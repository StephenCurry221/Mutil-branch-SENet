...
def getGradient(input, show=True):
    ...
    if show:
        ...
        axes[0, 1].imshow(np.squeeze(np.array(h))[0:size, 0:size], cmap='Accent_r')
        axes[1, 0].imshow(-np.squeeze(np.array(dv))[0:size, 0:size], cmap='jet')
        axes[1, 1].imshow(np.squeeze(np.array(v))[0:size, 0:size], cmap='Accent_r')
        axes[0, 0].set_title('Gradient', fontsize=10)
        axes[0, 0].set_ylabel('Horizontal', fontsize=10)
        axes[0, 1].set_title('Raw', fontsize=10)
...
class Data(Dataset):
    def __init__(self, root='/media/tiger/Disk0/jwang/skin/train/Images/',
                 cropSize=(512, 512),
        ...
        self.root = root
        # print(root)
        # self.imgs = os.listdir('/media/tiger/Disk0/jwang/skin/train/' + 'train_img')
    ...
    def __getitem__(self, item):
        root = '/media/tiger/Disk0/jwang/skin/train/Images'
        # imgPath = Path(os.path.join(root + self.imgs[item]))  # test
        # imgPath = Path('/media/tiger/Disk0/jwang/skin/train/train_img/' + self.imgs[item]) #train
        imgPath = os.path.join(root, self.imgs[item])
        maskPath = Path('/media/tiger/Disk0/jwang/skin/test/Annotation_instance/' + (self.imgs[item].split('.')[0] + '.npy')) # test
        # maskPath = Path('/media/tiger/Disk0/jwang/skin/train/Annotation_instance/' + (self.imgs[item].split('.')[0] + '.npy')) #train

        mask_cls_path = Path('/media/tiger/Disk0/jwang/skin/train/Annotation_cls/' + (self.imgs[item].split('.')[0] + '.npy'))

        verticalPath = Path(
            '/media/tiger/Disk0/jwang/skin/train/HorizontalVerticalMap/' + (self.imgs[item].split('.')[0] + '_vertical.npy'))
        horizontalPath = Path(
            '/media/tiger/Disk0/jwang/skin/train/HorizontalVerticalMap/' + (self.imgs[item].split('.')[0] + '_horizontal.npy'))
        if self.mode == 'train':
            # img = Image.open(imgPath).convert('RGB')
            img = Image.open(imgPath)
            mask = np.load(maskPath)
            mask[mask > 0] = 1
            # mask_se = np.load(maskPath)[..., -1]
            # print(mask_se)
            mask_cls = np.load(mask_cls_path)
            ...
            if self.crop:
                while True:
                    ...
                    # mask_se_ = transform.apply(np.array(mask_se), h_start=wh['h_start'], w_start=wh['w_start'])
                    mask_cls_ = transform.apply(np.array(mask_cls), h_start=wh['h_start'], w_start=wh['w_start'])
                    # if len(np.unique(mask_)) == 2:
                    img, vertical, horizontal, mask, mask_cls = img_, vertical_, horizontal_, mask_, mask_cls_
            ...
            # mask_se = np.array(mask_se, dtype=np.float32)
            mask_cls = np.array(mask_cls, dtype=np.float32)
            # assert (len(np.unique(mask)) == 2)
            horizontalVertical = np.concatenate((vertical, horizontal), axis=-1)
            return self.toTensor(img), mask[None, ...], np.transpose(horizontalVertical, (2, 0, 1)), mask_cls[None, ...]
            ...
            return self.toTensor(img), self.imgs[item]
            # if maskPath.exists():
...
if __name__ == '__main__':
    ...
    #     plt.show()
    #     break
    # data = Data(root='/media/tiger/Disk0/jwang/skin/train/',
    #             mode='train',
    #             isAugmentation=False)
    # for i in data:
    #     output = i[0]
