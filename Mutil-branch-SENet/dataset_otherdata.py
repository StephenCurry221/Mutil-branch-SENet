...
class Data(Dataset):
    def __init__(self, root="/media/tiger/Disk0/jwang/skin/",
                 cropSize=(512, 512),
        ...
        self.root = root
        # print(root)
        # self.imgs = os.listdir('/home/cliu/PycharmProjects/CGC-Net/data/raw/colorectal/' + 'fold_1/Grade1')
        # self.imgs = os.listdir('/media/tiger/Disk0/jwang/country/data/')
        self.imgs = os.listdir('/media/tiger/Disk0/jwang/skin/test/test_other')
    ...
    def __getitem__(self, item):
        # imgPath = Path('/media/tiger/Disk0/jwang/country/data/' + self.imgs[item])
        imgPath = Path('/media/tiger/Disk0/jwang/skin/test/test_other/' + self.imgs[item])
        # maskPath = Path('./skin/Annotation_instance/' + (self.imgs[item].split('.')[0] + '.npy'))

        mask_cls_path = Path('/media/tiger/Disk0/jwang/skin/train/Annotation_cls/' + (self.imgs[item].split('.')[0] + '.npy'))
        verticalPath = Path(
            './skin/HorizontalVerticalMap/' + (self.imgs[item].split('.')[0] + '_vertical.npy'))
        horizontalPath = Path(
            './skin/HorizontalVerticalMap/' + (self.imgs[item].split('.')[0] + '_horizontal.npy'))
        if self.mode == 'train':
            ...
            return self.toTensor(img), mask_se[None, ...], np.transpose(horizontalVertical, (2, 0, 1)), mask_cls[None, ...]
