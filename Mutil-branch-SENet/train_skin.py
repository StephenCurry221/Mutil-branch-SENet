...
def main(args, logger):
    ...
    for epoch in range(args.epoch):
        ...
        for img, mask, horizontalVertical, mask_cls in trainLoader:
            iter += 1
            img, mask, horizontalVertical, mask_cls = img.to(device), mask.to(device), horizontalVertical.to(device), mask_cls.to(device)
            optimizer.zero_grad()
            [branchSeg, branchMSE, branchse] = net(img)
            predictionGradient = getGradient(branchMSE, show=False)
            gtGradient = getGradient(horizontalVertical, show=False)
            ...
            loss4 = criterionMCE(branchse, mask_cls.long().squeeze())
            # loss5 = Dice(branchse, mask_cls)
